...
class Game extends UI {
    ...
    #newGame(rows = this.#config.easy.rows, cols = this.#config.easy.cols, mines = this.#config.easy.mines) {
        ...
        if (!isWin) {
            this.#revealMines();
            this.#modal.infoText = `You lost. Try Agian!`;
        ...
        this.#modal.infoText = this.#timer.numberOfSeconds < this.#timer.maxNumberOfSeconds ? `You won! It took you ${this.#timer.numberOfSeconds} seconds. Congratulations!` : `You won! Congratulation!`;
    ...
    #setCellValue(cell) {
        ...
        for (let rowIndex = Math.max(0, cell.y - 1); rowIndex <= Math.min(cell.y + 1, this.#numberOfRows - 1); rowIndex++) {
            for (let colIndex = Math.max(0, cell.x - 1); colIndex <= Math.min(cell.x + 1, this.#numberOfCols - 1); colIndex++) {
